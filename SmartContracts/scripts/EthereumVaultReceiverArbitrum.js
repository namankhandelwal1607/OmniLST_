...
async function main() {
  ...
  const EthereumVaultReceiverArbitrum = await ethers.getContractFactory("EthereumVaultReceiverArbitrum");
  const ethereumVaultReceiverArbitrum = await EthereumVaultReceiverArbitrum.deploy(
  ...
  await ethereumVaultReceiverArbitrum.deployed();


  return ethereumVaultReceiverArbitrum.address;
}
